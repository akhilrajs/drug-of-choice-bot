...
def start(update, context):
    ...
    update.message.reply_text(''' Welcome to Drug Of Choice bot by akhil_raj_s_
...
To get the classifcation of drugs table
for example : "/classify beta blockers"  to get the table of beta blocker classification.

To see the index of classification go to "MENU" on the bottom left corner and click "/classification_index".

Click on any title to copy automatically and paste it and send it, i will send you the classification table.

The database is updated according to the latest syllabus according to the NEXT exam ''')
    token = "token_here"
    bot = telepot.Bot(token)
    bot.sendVideo(chat_id=user_id, video="https://user-images.githubusercontent.com/67222042/126078114-cca107c5-56fb-4d23-8b1e-4088901d7aa3.mp4", supports_streaming=True, caption = "The complete instruction manual for the bot")
...
''')
    if search_result == "NOT FOUND":
          update.message.reply_text('''Not found in Database.
It could be due to :
1) not added to database yet
2) not the drug of choice for any condition
3) spelling mistake

''')
          ListOfClosestDrugs=get_close_matches(search_keyword, doc,cutoff=0.1)
          for i in ListOfClosestDrugs:
              indx_no = doc.index(i)
              update.message.reply_text(f'''closest match in drugs is :

Drug : {i}

Condition of choice : {condition[indx_no]}
